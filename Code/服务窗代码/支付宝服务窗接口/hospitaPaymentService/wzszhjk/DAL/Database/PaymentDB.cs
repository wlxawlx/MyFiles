...
namespace HospitaPaymentService.Wzszhjk.DAL.Database
{
    public class PaymentDB : BaseDB
    {
        ...
        public int DB_QueryQrderList(string brid, string brlx, string type, string querytype, int pageno, int pagenum, out ArrayList values, out string msg)
        {
            int ret = -1;
            values = null;
            msg = "";
            ...
            try
            {
                ...
                string exeSql = _builder.OrderListSql(type, pageno, pagenum, querytype, brlx, curBrid, out  flag, out  msg);
                if (!flag)
                {
                    return 10;
                }
        ...
        /// <param name="msg">出错信息</param>
        /// <returns>0：成功  大于0：出错  小于0：异常</returns>
        public int DB_CreateOrder(string brid, string bkhm, double czje, double tkje, string brlx, string payType, out long yylsh, out string msg)
        {
            msg = "";
            yylsh = -1;
...
" czsj : " + _cssj.ToString("yyyy-MM-dd HH:mm:ss") + " brlx : " + brlx + " 。";
            ...
            try
            {
                string insertSql = _builder.CreateOrderSql(yylsh, czje, tkje, brlx, bkhm, curBrid, _cssj, _sfzh, _lxdh, _brxm, payType);
        ...
        /// <param name="jysj">交款时间</param>
        /// <param name="yylsh">医院流水号</param>
        /// <param name="msg"></param>
        /// <returns>0-成功   大于0-失败   小于0-异常</returns>
        private int DB_UpdateHisCharge(OracleConnection connection, OracleTransaction tx, string yhlsh, double je, string brid, string brlx, DateTime jysj, out string msg)
        {
            int ret = -1;
            msg = "";
            if (brlx.Equals("1"))
            ...
            else if (brlx.Equals("2"))
            {
                ...
                int result = -99;
                OracleDataReader dataReader = DbHelperOra.ExeProcedureUseTran(connection, tx, WebConfigParameter.ProcedureName, parameters, out result);
        ...
        /// <summary>
        /// 读取数据库的系统时间(默认值1900-01-01 00:00:00)
        /// </summary>
        ...
        private bool IsSupportPayment(string brlx)
        {
            ...
            if (WebConfigParameter.HospitalName() == AppUtils.HOSPITALNAME.WZSDEYY || WebConfigParameter.HospitalName() == AppUtils.HOSPITALNAME.WZSTXRMYY)
            {
                result = true;
            }
            else if (brlx.Equals("1") &&
                  (WebConfigParameter.HospitalName() == AppUtils.HOSPITALNAME.WZSZYY || WebConfigParameter.HospitalName() == AppUtils.HOSPITALNAME.WZSZXYJHYY || WebConfigParameter.HospitalName() == AppUtils.HOSPITALNAME.WZSDSRMYY))
            {
                result = true;
            }
            else if (brlx.Equals("2") && WebConfigParameter.HospitalName() == AppUtils.HOSPITALNAME.WZSDQRMYY)
            {
                result = true;
            }
