...
{
    //温州市苍南县妇幼保健医院
    public class SQLUtilsWZSCNXFYBJYY : SQLUtils
    {
        public SQLUtilsWZSCNXFYBJYY()
        {
            ...
            zybrQueryStr = "select distinct zy.bingrenbh brid, (case when gy.ybkh is null then gy.jzkh else gy.ybkh end) bkhm," + 
                           "(case when length(gy.ybkh)=41 then 'smk' when length(gy.ybkh)=20 then 'ybk' else 'byk' end) bklx," +
