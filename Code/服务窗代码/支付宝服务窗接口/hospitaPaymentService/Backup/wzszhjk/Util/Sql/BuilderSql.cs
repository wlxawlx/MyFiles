...
namespace HospitaPaymentService.Wzszhjk.Utils.SqlString
{
    ...
    public class BuilderSql
    {
        ...
        /// <param name="msg"></param>
        /// <returns></returns>
        public string OrderListSql(string type, int pageno, int pagenum, string querytype, string brlx, string brid,
            out bool flag, out string msg)
        {
            flag = true;
            msg = "";
        ...
        /// <param name="openid"></param>
        /// <returns></returns>
        public string CreateOrderforAlipaySql(long yylsh, double czje, double tkje, string brlx, string bkhm, string brid, DateTime cssj,
            string sfzh, string lxdh, string brxm, string payType, string openid)
        {
            string result = "insert into " + YlddTableName() + "(yylsh, czje, tkje, ddzt, jyfs, bkhm, brid, cssj, czsj, brlx, brxm, sfzh, lxdh, paytype, openid) values( '" +
                    yylsh + "', " + czje + ", " + tkje + ", 0, '" + brlx + "', '" + bkhm + "', '" + brid + "', " + "to_date('" +
                    cssj.ToString("yyyy-MM-dd HH:mm:ss") + "','yyyy-mm-dd hh24:mi:ss')," +
                    "to_date('" + cssj.ToString("yyyy-MM-dd HH:mm:ss") + "','yyyy-mm-dd hh24:mi:ss'), '" + brlx +
                    "', '" + brxm + "', '" + sfzh + "', '" + lxdh + "', '" + payType + "', '" + openid + "')";
        ...
        public string UpdateHisSql(string brid, double zhye, double czje, DateTime jysj, string pzhm, YLReplyInfo info)
        {
            ...
            // 多一个日期
            if (hospitalName == AppUtils.HOSPITALNAME.WZSZXYJHYY ||
               hospitalName == AppUtils.HOSPITALNAME.WZSDSRMYY ||
                hospitalName == AppUtils.HOSPITALNAME.WZSDEYY)
            {
                result += ", jzrq";
            }
